...
void sendResponse(const char *id, const char *body) {
    amqp_basic_properties_t props;
    memset(&props, 0, sizeof(props));
    props._flags = AMQP_BASIC_CORRELATION_ID_FLAG;
    props.correlation_id = amqp_cstring_bytes(id);

    amqp_basic_publish(
        connection, 
        channel, 
        amqp_empty_bytes, 
        amqp_cstring_bytes("control"),
        0, 
        0, 
        &props, 
